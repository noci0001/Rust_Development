...
fn main() {
    ...
    // Create a closure that returns the square of an integer (the
    ...
    let pairs_second = vec![(0, 1), (2, 3), (4, 5)];

    for mut pair in pairs_second.into_iter() {
        pair.0 += 1;
        println!("{:?}", pair);
    }

    let mut numbers_second = vec![1, 2, 3, 4];

    //    for x in numbers.iter_mut() {
    //        *x *= 3 // multiply the value by 3 via the mutable reference x
    //    }

    let n: Vec<_> = numbers_second
        .into_iter()
        .map(|mut num| num * 3)
        .collect::<Vec<_>>();
